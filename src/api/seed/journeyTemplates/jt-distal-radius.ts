import type { JourneyTemplate } from '../../schemas'
import { daysAgo, iso } from '../shared'

export const jtDistalRadius: JourneyTemplate = {
  id: 'jt-distal-radius',
  name: 'Distala radiusfraktur',
  description:
    'Härledd från Standardfraktur — handledsspecifika fysioterapiinstruktioner med gipsborttagning vid vecka 4',
  parentTemplateId: 'jt-standard',
  derivedAt: iso(daysAgo(60)),
  entries: [
    {
      id: 'jte-dr-1',
      label: 'Dag 1–2',
      offsetDays: 1,
      windowDays: 1,
      order: 1,
      templateId: 'qt-numbness-infection',
      dashboardCategory: 'ACUTE',
      scoreAliases: { PNRS_1: 'PNRS_baseline' },
      scoreAliasLabels: { PNRS_baseline: 'VAS (PNRS) vid dag 1–2' },
    },
    {
      id: 'jte-dr-2',
      label: 'Dag 10–14 – stygnborttagning',
      offsetDays: 10,
      windowDays: 4,
      order: 2,
      templateId: 'qt-wound-pain',
      dashboardCategory: 'ACUTE',
      scoreAliases: { PNRS_2: 'PNRS_day14' },
      scoreAliasLabels: { PNRS_day14: 'VAS (PNRS) vid dag 10–14' },
    },
    {
      id: 'jte-dr-3',
      label: 'Vecka 4 – gipsborttagning och fysioterapi',
      offsetDays: 28,
      windowDays: 3,
      order: 3,
      templateId: 'qt-function-prwe-short',
      dashboardCategory: 'SUBACUTE',
      scoreAliases: { PNRS_2: 'PNRS_week4', 'PRWE.total': 'PRWE_week4' },
      scoreAliasLabels: {
        PNRS_week4: 'VAS (PNRS) vid vecka 4',
        PRWE_week4: 'PRWE-short vid vecka 4',
      },
    },
    {
      id: 'jte-dr-4',
      label: 'Vecka 6–8 – styrketräning',
      offsetDays: 49,
      windowDays: 7,
      order: 4,
      templateId: 'qt-function-prwe-short',
      dashboardCategory: 'SUBACUTE',
      scoreAliases: { PNRS_2: 'PNRS_week8', 'PRWE.total': 'PRWE_week8' },
      scoreAliasLabels: {
        PNRS_week8: 'VAS (PNRS) vid vecka 6–8',
        PRWE_week8: 'PRWE-short vid vecka 6–8',
      },
    },
    {
      id: 'jte-dr-5',
      label: '6 månader',
      offsetDays: 180,
      windowDays: 14,
      order: 5,
      templateId: 'qt-eq5d-prwe-short',
      dashboardCategory: 'CONTROL',
      scoreAliases: { 'PRWE.total': 'PRWE_6m', 'EQ5D.index': 'EQ5D_6m', EQ_VAS: 'EQ_VAS_6m' },
      scoreAliasLabels: {
        PRWE_6m: 'PRWE-short vid 6 månader',
        EQ5D_6m: 'EQ-5D index vid 6 månader',
        EQ_VAS_6m: 'EQ VAS vid 6 månader',
      },
    },
    {
      id: 'jte-dr-6',
      label: '1 år',
      offsetDays: 365,
      windowDays: 14,
      order: 6,
      templateId: 'qt-eq5d-prwe-short',
      dashboardCategory: 'CONTROL',
      scoreAliases: { 'PRWE.total': 'PRWE_1yr', 'EQ5D.index': 'EQ5D_1yr', EQ_VAS: 'EQ_VAS_1yr' },
      scoreAliasLabels: {
        PRWE_1yr: 'PRWE-short vid 1 år',
        EQ5D_1yr: 'EQ-5D index vid 1 år',
        EQ_VAS_1yr: 'EQ VAS vid 1 år',
      },
    },
  ],
  instructions: [
    {
      id: 'jti-dr-1',
      journeyTemplateId: 'jt-distal-radius',
      instructionTemplateId: 'it-wound-care',
      label: 'Dag 1–2: Sårvård',
      startDayOffset: 1,
      endDayOffset: 14,
      order: 1,
      tags: ['wound', 'acute'],
    },
    {
      id: 'jti-dr-2',
      journeyTemplateId: 'jt-distal-radius',
      instructionTemplateId: 'it-distal-radius',
      label: 'Dag 10–14: Handledsfysioterapi',
      startDayOffset: 10,
      endDayOffset: 28,
      order: 2,
      tags: ['wrist', 'physio'],
    },
    {
      id: 'jti-dr-3',
      journeyTemplateId: 'jt-distal-radius',
      instructionTemplateId: 'it-distal-radius',
      label: 'Vecka 4: Gipsborttagning och mobilisering',
      startDayOffset: 28,
      endDayOffset: 49,
      order: 3,
      tags: ['wrist', 'physio'],
    },
    {
      id: 'jti-dr-4',
      journeyTemplateId: 'jt-distal-radius',
      instructionTemplateId: 'it-distal-radius',
      label: 'Vecka 6–8: Styrketräning',
      startDayOffset: 49,
      endDayOffset: 84,
      order: 4,
      tags: ['wrist', 'physio'],
    },
  ],
  createdAt: iso(daysAgo(60)),
  referenceDateLabel: 'Operationsdatum',
}
