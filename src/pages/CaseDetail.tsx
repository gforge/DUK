...
import { Alert, Badge, Box, Breadcrumbs, Chip, Link, Paper, Skeleton, Stack, Tab, Tabs, Typography, } from '@mui/material';
...
function TabPanel({ children, value, index }: TabPanelProps) {
    return (<div role="tabpanel" hidden={value !== index} id={`case-tabpanel-${index}`} aria-labelledby={`case-tab-${index}`}>
...
export default function CaseDetail() {
    ...
    const { data: caseData, loading: caseLoading, error: caseError, refetch: refetchCase, } = useApi(() => client.getCase(id!), [id]);
    const { data: patient, loading: patientLoading } = useApi(() => (caseData ? client.getPatient(caseData.patientId) : Promise.resolve(undefined)), [caseData?.patientId]);
    useEffect(() => {
    ...
    const handlePatientBack = useCallback(() => navigate(caseData ? `/patients/${caseData.patientId}` : '/patients'), [navigate, caseData]);
    ...
    return (<Box>
      ...
      <Breadcrumbs aria-label="breadcrumb" sx={{ mb: 1 }}>
        <Link component="button" variant="body2" onClick={() => navigate('/dashboard')} underline="hover" sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
      ...
      <Stack direction="row" sx={{ alignItems: 'center', gap: 2, mb: 2, flexWrap: 'wrap' }}>
        ...
        {caseData.policyWarnings.length > 0 && (<AutoWarningsBadge warnings={caseData.policyWarnings} lastActivityAt={caseData.lastActivityAt}/>)}
      </Stack>
      ...
      <Paper variant="outlined" sx={{ mt: 2, borderRadius: 2 }}>
        <Tabs value={tabValue} onChange={(_, v) => {
        ...
        }} aria-label="case detail tabs" variant="scrollable" scrollButtons="auto" sx={{ borderBottom: 1, borderColor: 'divider' }}>
          ...
          <Tab label={<Badge color="error" variant="dot" invisible={!['NEW', 'NEEDS_REVIEW'].includes(caseData.status)} sx={{ pr: 1 }}>
                {t('case.tab_triage')}
              </Badge>} id="case-tab-2" aria-controls="case-tabpanel-2"/>
        ...
        <Box sx={{ p: 2 }}>
          ...
          <TabPanel value={tabValue} index={2}>
            <TriageTab caseData={caseData} onTriaged={refetchCase} routeContactMode={routeContactMode}/>
          </TabPanel>

          <TabPanel value={tabValue} index={3}>
            <JournalTab caseData={caseData} patient={patient ?? undefined} onCaseChange={refetchCase}/>
          </TabPanel>
