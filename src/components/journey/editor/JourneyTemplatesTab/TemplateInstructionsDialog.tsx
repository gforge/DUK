...
export function TemplateInstructionsDialog({ template, instructionTemplates, onClose, onSaved, }: Readonly<Props>) {
    const { t } = useTranslation();
    const { showSnack } = useSnack();
    const [saving, setSaving] = useState(false);
    const [rows, setRows] = useState<JourneyTemplateInstruction[]>(() => template.instructions
        .slice()
        .sort((a, b) => a.order - b.order)
        .map((i) => ({ ...i })));
    const templateOptions = useMemo(() => instructionTemplates.map((it) => ({ id: it.id, name: it.name })), [instructionTemplates]);
    const addRow = () => {
        setRows((prev) => [
            ...prev,
            {
    ...
    return (<Dialog open onClose={onClose} fullWidth sx={{ maxWidth: 'md' }}>
      ...
      <DialogContent>
        <Stack sx={{ mt: 1, gap: 1.5 }}>
          {rows.length === 0 && (<Typography variant="body2" color="text.secondary">
              {t('journey.noInstructions')}
            </Typography>)}

          {rows.map((row) => (<Stack key={row.id} direction="row" sx={{ gap: 1, alignItems: 'center' }}>
              <FormControl size="small" sx={{ minWidth: 220 }}>
                <InputLabel>{t('journey.entry.instructionFromTemplate')}</InputLabel>
                <Select label={t('journey.entry.instructionFromTemplate')} value={row.instructionTemplateId} onChange={(e) => patchRow(row.id, { instructionTemplateId: e.target.value })}>
                  {templateOptions.map((opt) => (<MenuItem key={opt.id} value={opt.id}>
                      {opt.name}
                    </MenuItem>))}
                </Select>
              </FormControl>

              <TextField size="small" label={t('journey.entry.label')} value={row.label ?? ''} onChange={(e) => patchRow(row.id, { label: e.target.value })} sx={{ minWidth: 220 }}/>

              <TextField size="small" type="number" label={t('journey.offsetDays')} value={row.startDayOffset} onChange={(e) => patchRow(row.id, { startDayOffset: Number(e.target.value) })} sx={{ width: 120 }}/>

              <TextField size="small" type="number" label={t('journey.window')} value={row.endDayOffset ?? ''} onChange={(e) => patchRow(row.id, {
