...
type MergedRow = {
    kind: 'entry';
    item: JourneyTemplateEntry;
} | {
    kind: 'instruction';
    item: JourneyTemplateInstruction;
};
...
interface Props {
    ...
    setEntryEditState: React.Dispatch<React.SetStateAction<{
...
export function TemplateAccordion({ jt, instructionTemplates, parentName, onDelete, setSyncTarget, setEditTarget, setDeriveTarget, setTemplateInstructionsTarget, setEntryEditState, handleDeleteEntry, handleDeleteInstruction, }: Props) {
    ...
    return (<Accordion variant="outlined" sx={{ borderRadius: '8px !important' }}>
        ...
        <Stack direction="row" sx={{ flex: 1, pr: 1, alignItems: 'center', gap: 1.5 }}>
          ...
          <Box sx={{ flex: 1 }}>
            <Stack direction="row" sx={{ alignItems: 'center', gap: 0.75 }}>
              ...
              {jt.parentTemplateId && (<Chip label={`${t('journey.editor.derived')} ← ${parentName(jt.parentTemplateId) ?? '?'}`} size="small" color="info" variant="outlined" sx={{ fontSize: 10, height: 20 }}/>)}
            </Stack>
            {jt.description && (<Typography variant="caption" color="text.secondary">
                {jt.description}
              </Typography>)}
          </Box>
          <Chip label={`${jt.entries.length} ${t('journey.steps')}`} size="small" variant="outlined"/>
          {jt.instructions.length > 0 && (<Chip label={`${jt.instructions.length} ${t('journey.editor.tabInstructions')}`} size="small" variant="outlined"/>)}
          {jt.parentTemplateId && (<Tooltip title={t('journey.editor.syncFromParent')}>
      ...
      <AccordionDetails sx={{ pt: 0 }}>
        <Box sx={{ overflowX: 'auto' }}>
          <Table size="small">
            ...
            <TableBody>
              {getMergedRows(jt).map((row) => row.kind === 'entry' ? (<EntryRow key={`entry-${row.item.id}`} jt={jt} entry={row.item} setEntryEditState={setEntryEditState} handleDeleteEntry={handleDeleteEntry}/>) : (<InstructionRow key={`instr-${row.item.id}`} jt={jt} instr={row.item} instrName={instrNameMap.get(row.item.instructionTemplateId) ??
                row.item.instructionTemplateId} instrContent={instrContentMap.get(row.item.instructionTemplateId) ?? ''} setTemplateInstructionsTarget={setTemplateInstructionsTarget} handleDeleteInstruction={handleDeleteInstruction}/>))}
            </TableBody>
          </Table>
        </Box>
      </AccordionDetails>
