    ...
    return (<TableRow hover>
      <TableCell sx={{ minWidth: 120 }}>
        <Stack direction="row" sx={{ alignItems: 'center', gap: 0.5 }}>
          <JourneyIcon icon={entry.icon} color="action" fontSize="small" />
          <Typography variant="body2" sx={{ fontWeight: 600 }}>
            {entry.label}
          </Typography>
        </Stack>
        {entry.stepKey && (<Typography variant="caption" sx={{ fontFamily: 'monospace', color: 'text.secondary', display: 'block', mt: 0.25 }}>
            {entry.stepKey}
          </Typography>)}
        {entry.recurrenceIntervalDays !== undefined && (<Chip icon={<RepeatIcon />} label={t('journey.template.recurrenceUnit', { count: entry.recurrenceIntervalDays })} size="small" color="secondary" variant="outlined" sx={{ fontSize: 10, height: 18, mt: 0.5 }}/>)}
      </TableCell>
