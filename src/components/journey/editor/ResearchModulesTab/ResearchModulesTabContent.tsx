...
export function ResearchModulesTab({ researchModules, loading, questionnaires, onDelete, onSave, }: Props) {
    ...
    return (<>
      <Stack direction="row" sx={{ mb: 1.5, justifyContent: 'flex-end' }}>
        <Button size="small" variant="outlined" startIcon={<AddIcon />} onClick={() => setEditTarget(undefined)}>
          {t('journey.research.createModule')}
        </Button>
      </Stack>

      {!researchModules?.length ? (<Typography color="text.secondary">{t('journey.editor.noModules')}</Typography>) : (<Stack sx={{ gap: 1.5 }}>
          {researchModules.map((rm) => (<Accordion key={rm.id} variant="outlined" sx={{ borderRadius: '8px !important' }}>
              <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                <Stack direction="row" sx={{ flex: 1, pr: 1, alignItems: 'center', gap: 1.5 }}>
                  ...
                  <Chip label={`${rm.entries.length} ${t('journey.entries')}`} size="small" color="secondary" variant="outlined"/>
              ...
              <AccordionDetails sx={{ pt: 0 }}>
                <Table size="small">
                  ...
                  <TableBody>
                    {rm.entries.map((entry) => (<TableRow key={entry.id} hover>
                        <TableCell>
                        ...
                        <TableCell>
                          {entry.replaceStepId ? (<Chip label={`${t('journey.research.replaces')} ${entry.replaceStepId}`} size="small" color="warning" variant="outlined" sx={{ fontSize: 10, height: 20 }}/>) : (<Chip label={`${t('journey.research.additive')} (day ${entry.offsetDays})`} size="small" color="secondary" variant="outlined" sx={{ fontSize: 10, height: 20 }}/>)}
                        </TableCell>
                        <TableCell>
      ...
      {editTarget !== null && (<ModuleEditorDialog module={editTarget ?? undefined} questionnaires={questionnaires ?? []} onSave={(data: any) => {
                onSave(data);
                setEditTarget(null);
