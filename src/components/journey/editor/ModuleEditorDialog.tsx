...
interface ModuleEditorDialogProps {
    ...
    onSave: (m: Omit<ResearchModule, 'id' | 'createdAt'> & {
        id?: string;
    }) => void;
    onClose: () => void;
}
export default function ModuleEditorDialog({ module, questionnaires, onSave, onClose, }: ModuleEditorDialogProps) {
    ...
    const [entryDrafts, setEntryDrafts] = useState<EntryDraft[]>(module ? moduleToEntryDrafts(module) : []);
      ...
      <DialogContent>
        <Stack sx={{ mt: 0.5, gap: 2 }}>
          <Stack direction="row" sx={{ gap: 2 }}>
            <TextField label={t('journey.template.name')} value={name} onChange={(e) => setName(e.target.value)} size="small" required fullWidth autoFocus/>
            <TextField label={t('journey.research.studyName')} value={studyName} onChange={(e) => setStudyName(e.target.value)} size="small" required fullWidth/>
          </Stack>

          <TextField label={t('journey.research.studyInfo')} value={studyInfoMarkdown} onChange={(e) => setStudyInfoMarkdown(e.target.value)} size="small" fullWidth multiline minRows={3} helperText={t('journey.research.studyInfoHint')}/>
          ...
          {entryDrafts.length === 0 ? (<Typography variant="body2" color="text.secondary">
              ...
              <TableBody>
                {entryDrafts.map((e) => (<TableRow key={e._id}>
                    <TableCell>
                      <TextField value={e.label} onChange={(ev) => updateEntry(e._id, 'label', ev.target.value)} size="small" variant="standard" placeholder="Dag 14" sx={{ minWidth: 90 }}/>
                    </TableCell>
                    <TableCell>
                      <Autocomplete options={qtOptions} value={qtOptions.find((q) => q.id === e.templateId) ?? null} getOptionLabel={(o: any) => o.name} isOptionEqualToValue={(o: any, v: any) => o.id === v.id} onChange={(_event: any, val: any | null) => updateEntry(e._id, 'templateId', val?.id ?? '')} size="small" sx={{ minWidth: 160 }} renderInput={(params: any) => (<TextField {...params} size="small" variant="standard"/>)}/>
                    </TableCell>
                    <TableCell>
                      <ToggleButtonGroup size="small" exclusive value={e.mode} onChange={(_, v) => {
                    if (v)
                        updateEntry(e._id, 'mode', v as EntryMode);
                }}>
                    ...
                    <TableCell>
                      {e.mode === 'REPLACE' ? (<TextField value={e.replaceStepId} onChange={(ev) => updateEntry(e._id, 'replaceStepId', ev.target.value)} size="small" variant="standard" placeholder="step-id" sx={{ minWidth: 100 }} helperText={t('journey.research.replaceStepHint')}/>) : (<TextField value={e.offsetDays} type="number" onChange={(ev) => updateEntry(e._id, 'offsetDays', ev.target.value !== '' ? Number(ev.target.value) : '')} size="small" variant="standard" label={t('journey.entry.offsetDays')} sx={{ width: 80 }}/>)}
                    </TableCell>
                    <TableCell>
