...
export default function SyncFromParentDialog({ childTemplate, onClose, onSynced }: Props) {
    ...
    return (<Dialog open onClose={onClose} fullWidth sx={{ maxWidth: 'sm' }}>
      <DialogTitle>{t('journey.editor.syncDialogTitle')}</DialogTitle>
      <DialogContent>
        {loadingDiff ? (<Skeleton variant="rectangular" sx={{ height: 100 }}/>) : !diffs || diffs.length === 0 ? (<Typography color="text.secondary">{t('journey.editor.syncNoChanges')}</Typography>) : (<Stack sx={{ mt: 1, gap: 1 }}>
            {diffs.map((d) => (<FormControlLabel key={d.entryId} control={<Checkbox checked={selected.has(d.entryId)} onChange={() => toggle(d.entryId)} size="small"/>} label={<Stack direction="row" sx={{ alignItems: "center", gap: 1 }}>
                    <Chip label={diffLabel[d.type]} size="small" color={diffColor[d.type]} sx={{ fontSize: 10, height: 20 }}/>
      ...
      <DialogActions sx={{ px: 2, pb: 2 }}>
        ...
        <Button variant="contained" onClick={handleApply} disabled={applying || !diffs?.length || selected.size === 0} disableElevation>
          {t('common.confirm')}
        </Button>
      </DialogActions>
    </Dialog>);
