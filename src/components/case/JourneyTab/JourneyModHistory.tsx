...
const MOD_ICON: Record<string, React.ReactNode> = {
    ...
    CANCEL: <CancelIcon color="error" fontSize="small" />,
};
interface Props {
...
export default function JourneyModHistory({ modifications }: Props) {
    ...
    return (<Box>
      ...
      <Stack sx={{ gap: 1 }}>
        {modifications.map((mod) => (<Paper key={mod.id} variant="outlined" sx={{ p: 1.5, borderRadius: 1 }}>
            ...
            {mod.entry && (<Typography variant="caption" sx={{ display: 'block' }}>
