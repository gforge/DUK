...
export default function BookingsList({ caseData, onChange }: Props) {
    ...
    const { data: responsiblePhysicianUserId } = useApi(() => client.getResponsiblePhysicianUserIdForCase(caseData.id), [caseData.id]);
    ...
    function toGoogleCalendarLink(b: {
        scheduledAt: string;
        type: string;
        note?: string;
    }) {
    ...
    async function handleSave() {
        ...
        try {
            await client.updateBooking(caseData.id, editing.id, { scheduledAt: editing.scheduledAt, role: editing.role as any, note: editing.note }, currentUser.id, currentUser.role);
    ...
    return (<Box>
      ...
      <Stack sx={{ gap: 1 }}>
        {(caseData.bookings ?? []).map((b) => (<Paper key={b.id} variant="outlined" sx={{ p: 1, display: 'flex', alignItems: 'center' }}>
            <Box sx={{ flex: 1 }}>
              ...
              {b.note && (<Typography sx={{ display: 'block' }} variant="caption">
                  {b.note}
                </Typography>)}
            </Box>
            ...
            <IconButton size="small" onClick={() => setCancelTarget(b.id)} aria-label={t('common.delete')}>
      ...
      <Dialog open={!!editing} onClose={() => setEditing(null)}>
        ...
        <DialogContent>
          <Stack sx={{ gap: 2, mt: 1 }}>
            <TextField label={t('triage.bookingTime')} type="datetime-local" value={editing?.scheduledAt ?? ''} onChange={(e) => setEditing((s) => (s ? { ...s, scheduledAt: e.target.value } : s))} slotProps={{ inputLabel: { shrink: true } }} size="small"/>
            <FormControl size="small">
              <InputLabel id="booking-role-label">{t('triage.assignRole')}</InputLabel>
              <Select labelId="booking-role-label" value={editing?.role ?? ''} onChange={(e) => setEditing((s) => (s ? { ...s, role: e.target.value } : s))} label={t('triage.assignRole')}>
                <MenuItem value="">
                  <em>{t('common.notSet')}</em>
                </MenuItem>
                <MenuItem value="NURSE">{t('role.NURSE')}</MenuItem>
                <MenuItem value="DOCTOR">{t('role.DOCTOR')}</MenuItem>
                <Tooltip title={isPalBookingAvailable ? '' : t('triage.bookingPalUnavailableTooltip')} disableHoverListener={isPalBookingAvailable} placement="right">
                  <span>
            ...
            <TextField label={t('triage.bookingNote')} value={editing?.note ?? ''} onChange={(e) => setEditing((s) => (s ? { ...s, note: e.target.value } : s))} multiline minRows={2} size="small"/>
          </Stack>
        </DialogContent>
        <DialogActions>
      ...
      <ConfirmDialog open={!!cancelTarget} title={t('triage.bookingCancelTitle')} message={t('triage.bookingCancelConfirm')} onConfirm={() => cancelTarget && handleCancel(cancelTarget)} onCancel={() => setCancelTarget(null)}/>
    </Box>);
}
