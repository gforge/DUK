      ...
      <Stack direction={{ xs: 'column', sm: 'row' }} sx={{ gap: 2 }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 1, flex: 1 }}>
          <Tooltip title={t('patients.openView')}>
            <Box onClick={() => navigate(`/patients/${patient.id}`)} sx={{
          ...
          <Box>
            ...
            {lastOpened ? (<Typography variant="caption" color="text.secondary">
                {t('patient.lastOpened')}: {lastOpened}
              </Typography>) : (<Chip label={t('trigger.NOT_OPENED')} size="small" color="warning" variant="outlined" sx={{ mt: 0.5 }}/>)}
          </Box>
        </Box>
        ...
        <Box sx={{ flex: 1 }}>
          <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
            ...
            {caseData.nextStep && (<Box sx={{ width: '45%', minWidth: 100 }}>
                <Typography variant="caption" color="text.secondary">
                  {t('triage.nextStep')}
                </Typography>
                <Typography variant="body2" sx={{ fontWeight: 600 }}>
            ...
            {caseData.deadline && (<Box sx={{ width: '45%', minWidth: 100 }}>
                <Typography variant="caption" color="text.secondary">
                  {t('case.deadline')}
                </Typography>
                <Box sx={{ mt: 0.25 }}>
        ...
        {(caseData.triggers.length > 0 || caseData.policyWarnings.length > 0) && (<>
            <Divider orientation="vertical" flexItem sx={{ display: { xs: 'none', sm: 'block' } }}/>
            <Box sx={{ flex: 1 }}>
              {caseData.triggers.length > 0 && (<Box sx={{ mb: 1 }}>
              ...
              {caseData.policyWarnings.length > 0 && (<Box>
                  <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mb: 0.5 }}>
                    {t('case.policyWarnings')}
                  </Typography>
                  <AutoWarningsBadge warnings={caseData.policyWarnings} lastActivityAt={caseData.lastActivityAt}/>
                </Box>)}
            </Box>
          </>)}
      </Stack>
