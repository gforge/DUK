...
export default function JournalDraftCard({ draft, canApprove, approving, copied, onApprove, onCopy, }: Props) {
    ...
    return (<Paper variant="outlined" sx={{ p: 2, borderRadius: 2 }}>
      <Stack sx={{ justifyContent: 'space-between', alignItems: 'center', mb: 1 }} direction="row">
        <Stack sx={{ gap: 1, alignItems: 'center' }} direction="row">
          <Chip label={draft.status === 'APPROVED' ? t('journal.approved') : t('journal.draft')} size="small" color={draft.status === 'APPROVED' ? 'success' : 'default'} icon={draft.status === 'APPROVED' ? <CheckCircleIcon fontSize="inherit" /> : undefined}/>
        ...
        <Stack sx={{ gap: 1 }} direction="row">
          <Tooltip title={copied === draft.id ? t('journal.copied') : t('journal.copy')}>
            <IconButton size="small" onClick={() => onCopy(draft.content, draft.id)} aria-label={t('journal.copy')}>
              <ContentCopyIcon fontSize="small" />
            </IconButton>
          </Tooltip>

          {draft.status === 'DRAFT' &&
            (canApprove ? (<Button size="small" variant="outlined" color="success" onClick={() => onApprove(draft.id)} disabled={approving === draft.id} startIcon={approving === draft.id ? <CircularProgress size={12}/> : undefined}>
                {t('journal.approve')}
              </Button>) : (<Tooltip title={t('journal.onlyDoctorApprove')}>
                <span>
      ...
      {draft.status === 'APPROVED' && (<Typography sx={{ display: 'block', mt: 1 }} variant="caption" color="text.secondary">
