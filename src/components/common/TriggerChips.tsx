...
export default function TriggerChips({ triggers, maxVisible = 3 }: TriggerChipsProps) {
    ...
    return (<Stack sx={{ flexWrap: 'wrap', gap: 0.5 }} direction="row" role="list" aria-label={t('dashboard.triggers')}>
      {visible.map((trigger) => {
            const cfg = TRIGGER_CONFIG[trigger];
            return (<Tooltip key={trigger} title={getTriggerLabel(trigger)} arrow enterDelay={300}>
            <Chip role="listitem" icon={cfg.icon} label={getTriggerLabel(trigger)} size="small" variant="outlined" color={cfg.color} aria-label={getTriggerLabel(trigger)} sx={{ fontSize: 10, height: 22 }}/>
          </Tooltip>);
        })}
      {overflow > 0 && (<Tooltip title={triggers
                ...
                .join(', ')} arrow>
          <Chip label={`+${overflow}`} size="small" variant="outlined" sx={{ fontSize: 10, height: 22 }}/>
        </Tooltip>)}
    </Stack>);
}
