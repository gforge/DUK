...
interface WorklistRowProps {
    ...
    onMarkDone: (caseId: string, options?: {
        bookingId?: string;
        followUpDate?: string;
        completionComment?: string;
    }) => Promise<void> | void;
}
export default function WorklistRow({ caseData, patient, assignedUserName, highlighted = false, onClaim, onMarkDone, }: WorklistRowProps) {
    ...
    const completionBooking = React.useMemo(() => {
        ...
        return (bookings.find((booking) => booking.status === 'SCHEDULED') ??
    ...
    return (<Box sx={{
            ...
            gap: 0.75,
            alignItems: 'center',
            borderLeft: (theme) => highlighted
                ? `2px solid ${alpha(theme.palette.primary.main, 0.4)}`
                : '2px solid transparent',
            transition: 'background-color 220ms ease-in-out, transform 220ms ease-in, opacity 220ms ease-in',
            transform: isCompleting ? 'translateX(16px)' : 'translateX(0)',
            opacity: isCompleting ? 0.12 : 1,
            '&:hover': {
                bgcolor: (theme) => highlighted ? alpha(theme.palette.primary.main, 0.03) : theme.palette.action.hover,
            },
        }}>
      {/* Patient + status */}
      <Stack sx={{ gap: 0.25 }}>
        ...
        <Stack direction="row" sx={{ gap: 0.5, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          {careRole && (<Chip icon={<CareRoleIcon role={careRole}/>} label={getCareRoleLabel(careRole)} size="small" sx={{ height: 18, fontSize: 11, color: 'text.secondary' }}/>)}
          {caseData.triageDecision?.assignmentMode && (<Tooltip title={caseData.triageDecision.assignmentMode === 'ANY'
                ? t('worklist.assignmentModeAnyTooltip', { defaultValue: 'Vem som helst' })
                : ''}>
              <Chip label={getAssignmentModeLabel(caseData.triageDecision.assignmentMode)} size="small" variant="outlined" sx={{ height: 18, fontSize: 11, color: 'text.secondary' }}/>
            </Tooltip>)}
          {assignedUserName && (<Chip label={`${t('case.assignedTo')}: ${assignedUserName}`} size="small" variant="outlined" sx={{ height: 18, fontSize: 11, color: 'text.secondary' }}/>)}
        </Stack>
      </Stack>

      {/* Deadline */}
      <Box>
        {caseData.deadline ? (<DeadlineLabel deadline={caseData.deadline} tone="queue"/>) : (<Typography variant="caption" color="text.disabled">
      ...
      <Box sx={{ display: { xs: 'none', sm: 'block' } }}>
        {caseData.internalNote ? (<Typography variant="caption" color="text.secondary" sx={{
                display: '-webkit-box',
                WebkitLineClamp: 2,
      ...
      <Stack direction="row" sx={{ gap: 0.5, justifyContent: 'flex-end', alignItems: 'center' }}>
        {/* Primary actions */}
        {(isTriaged || isFollowingUp) && (<Button size="small" variant="contained" color="primary" startIcon={<CheckCircleOutlineIcon />} onClick={() => setCompletionDialogOpen(true)} disabled={isCompleting} sx={{ fontSize: 12, whiteSpace: 'nowrap', fontWeight: 700 }}>
            {t('worklist.initiate')}
          </Button>)}

        {/* Secondary action */}
        {!caseData.assignedUserId && (<Button size="small" variant="outlined" startIcon={<AssignmentIndIcon />} onClick={() => onClaim(caseData.id)} sx={{ fontSize: 12, whiteSpace: 'nowrap' }}>
            {t('worklist.claim')}
          </Button>)}
