...
export default function PatientResponsibilityCard({ patientResponsiblePhysicianUserId, physicianOptions, onChange, }: Props) {
    ...
    return (<Paper variant="outlined" sx={{ p: 2.5, borderRadius: 2, mb: 3 }}>
      ...
      <FormControl size="small" sx={{ minWidth: 320, maxWidth: '100%' }}>
        ...
        <Select labelId="patient-responsible-physician-label" label={t('patientDetail.patientResponsiblePhysician')} value={patientResponsiblePhysicianUserId ?? ''} onChange={(e) => onChange(e.target.value ? String(e.target.value) : undefined)}>
          <MenuItem value="">
